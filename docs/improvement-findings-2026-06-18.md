...
> **注意**: `refactor-instructions.md`（第2サイクル）および `perf-batch-io-instructions.md` に
...
- [ ] **5-A** `ExpandedBitMonitorRow` のアドレス分割に `Split` を使用 — `MainWindowViewModel.cs:2012,2016`
- [ ] **5-B** `BuildPackedBits` が `BuildBitRows` の switch に含まれていない（デッドコード候補） — `BlockDataBuilder.cs:111-128`
- [ ] **5-C** `NormalizeNumericText` が 10進モードでも "0X" を除去 — `NumericFormatter.cs:92`
- [ ] **5-D** `CountLogRecordsAsync` の非効率な全行読み（初回起動時） — `FileLogStore.cs:275-282`
...
（`refactor-instructions.md` D1b に承認済み）
...
（`refactor-instructions.md` D2a に定義済み）
...
（`refactor-instructions.md` D6 に定義済み）
...
（`refactor-instructions.md` D2b に定義済み）
...
（`refactor-instructions.md` D3 に定義済み）
...
（`refactor-instructions.md` D2c に承認済み）
...
（`refactor-instructions.md` D11 に承認済み）
...
（`refactor-instructions.md` D10 に承認済み）
...
（`refactor-instructions.md` D10 に含む）
...
（`refactor-instructions.md` D9 に承認済み）
...
（`refactor-instructions.md` D12 に承認済み・特性テスト前提）
...
（`perf-batch-io-instructions.md` に詳細定義済み・第2サイクル完了後に着手予定）
...
## 7. 既承認タスク一覧（refactor-instructions.md 第2サイクル）
